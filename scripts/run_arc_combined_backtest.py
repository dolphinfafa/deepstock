...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument('--universe-dir',required=True); p.add_argument('--etf-prices',required=True); p.add_argument('--bull-targets',required=True); p.add_argument('--output-dir',required=True); p.add_argument('--confirmation-days',type=int,default=3); p.add_argument('--min-hold-days',type=int,default=5); p.add_argument('--rebalance-band',type=float,default=0.05); p.add_argument('--route-cooldown-days',type=int,default=5); a=p.parse_args()
    arc_config=ARCConfig(confirmation_days=a.confirmation_days,min_hold_days=a.min_hold_days)
    prices, _, _, routes = load_point_in_time_inputs(Path(a.universe_dir), Path(a.etf_prices))
    ...
    result.summary['confirmation_days']=a.confirmation_days; result.summary['min_hold_days']=a.min_hold_days; result.summary['rebalance_band']=a.rebalance_band; result.summary['route_cooldown_days']=a.route_cooldown_days
    (out/'summary.json').write_text(json.dumps(result.summary,indent=2,sort_keys=True),encoding='utf-8')
    (out/'manifest.json').write_text(json.dumps({'system_name':'Deepstock ARC','execution_status':ARC_EXECUTION_STATUS,'paper_authorized':False,'oos_parameter_selection':'prohibited','confirmation_days':a.confirmation_days,'min_hold_days':a.min_hold_days,'rebalance_band':a.rebalance_band,'route_cooldown_days':a.route_cooldown_days},indent=2,sort_keys=True),encoding='utf-8')
